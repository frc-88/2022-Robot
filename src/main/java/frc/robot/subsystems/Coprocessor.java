...
public class Coprocessor extends SubsystemBase {
    ...
    private final ROSInterface m_tunnel_interface;
    ...
    /** Creates a new Coprocessor. */
    public Coprocessor(Drive drive, WaypointMap waypointMap, ROSInterface tunnel_interface) {
        m_drive = drive;
        m_waypointMap = waypointMap;
        m_tunnel_interface = tunnel_interface;
    }

    public WaypointsPlan makeWaypointPlan() {
        return new WaypointsPlan(m_tunnel_interface);
    }
    ...
    private Pose2d getRobotPose() {
        return m_tunnel_interface.getGlobalPose();
    }
    ...
    public void updateFollowRosAuto() {
        ...
        switch (m_ros_auto_state) {
            ...
            case WAIT_FOR_RUNNING:
                if (m_tunnel_interface.getGoalStatus() == GoalStatus.RUNNING) {
            ...
            case WAIT_FOR_FINISHED:
                ...
                switch (m_tunnel_interface.getGoalStatus()) {
                    case RUNNING:
                        break;
                    case INVALID:
                    case IDLE:
